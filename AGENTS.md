...
- **后端**：Tauri 2 / Rust（edition 2024），Cargo workspace（成员为 `src-tauri`）；业务依赖 `chrono`（RFC3339 `Secs` 无毫秒，`dcterms:modified CCYY-MM-DDThh:mm:ssZ`）、`uuid`（`urn:uuid`）、`base64`（封面 data URL）、`regex`（卷章正则）、`encoding_rs`（content GBK 容错）、`chinese-number`（0.8.2 中文数字↔阿拉伯互转，`ChineseVariant::Simple` + `ChineseCountMethod::TenThousand`）、`zip`（EPUB 压缩，`mimetype` Stored 首条）、`epubveri`（0.9 EPUB3.3 校验，纯 Rust，AGPL-3.0）、`quick-xml`（0.41 2 空格 XML 美化）、`styloria`（0.11 CSS 美化，EPUB `base.css`）
...
- **构建命令**：`build` 域提供 `get_build`（`{ identifier }` → `BuildResult|null`） / `build_epub`（`{ identifier, chapterTitleFormat?, volumeTitleFormat?, numberFormat? }` → `BuildResult`，基于 `split.json+metadata.json` 生成 `build/<书名>/` 未压缩 EPUB，`numberFormat: arabic|arabic_padded|chinese_lower|chinese_upper`，`{order}/{title}` 占位，`sanitize_title` 去非法字符，落盘 `format.json` 供打包复用） / `remove_build`（`{ identifier }` → `bool`） / `read_build_file`（`{ identifier, relPath }` → `string`，仅文本文件） / `write_build_file`（`{ identifier, relPath, content }` → `bool`） / `get_build_path`（`{ identifier }` → `string|null` 供 `revealItemInDir`） / `get_format`（`{ identifier }` → `FormatConfig|null`） / `format_build_all`（`{ identifier }` → `FormatBuildResult`，整目录 2 空格格式化 `xhtml/opf/css`，`quick-xml` + `styloria`，失败不落盘，异步 `spawn_blocking`），前端经 `getBuild/buildEpub/removeBuild/readBuildFile/writeBuildFile/getBuildPath/getFormat/formatBuildAll`
